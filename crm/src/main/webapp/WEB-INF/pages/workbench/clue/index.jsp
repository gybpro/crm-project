...
<head>
    ...
    <script type="text/javascript">
        ...
        function toDetail(id) {
            window.location = "workbench/clue/detail.do?id=" + id;
        }

        // 查询刷新线索列表及数据
        function selectClueByConditionForPage(pageNo, pageSize) {
            let company = $.trim($("#select-company").val());
            let name = $.trim($("#select-name").val());
            let owner = $("#select-owner").val();
            let phone = $.trim($("#select-phone").val());
            let mPhone = $.trim($("#select-mPhone").val());
            ...
            $.ajax({
                ...
                success: function (json) {
                    ...
                    $.each(json.data.clueList, function (index, obj) {
                        ...
                        html += "<td>" + "<a onclick=\"toDetail('" + obj.id  + "')\">" + obj.fullName + obj.appellation +
                            "</a>" + "</td>";
